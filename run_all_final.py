...
% ── ABSTRACT ──
\begin{abstract}
We investigate whether the refusal direction identified by Arditi et al.\ (2024) encodes a general concept of harm or the act of being asked to produce harmful content. Using a six-condition experimental design across two Mistral model scales (7B, 12B), we report Cohen's $d(A,B)=""" + f"{d_ab}" + r"""$ and Cohen's $d(B,F)=""" + f"{d_bf}" + r"""$ at each scale, with pre-registered hypothesis test outcomes """ + f"H1={'/'.join(h1_list)}, H2={'/'.join(h2_list)}" + r""" and cross-model behavioral alignment (mean Pearson $r=""" + f"{avg_r:.3f}" + r"""$).
\end{abstract}
